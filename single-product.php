<?php
/**
 * Single Product Template — Garden Basket Hub
 */
?>
<!DOCTYPE html>
...
<head>
    ...
    <link rel="preload" as="style" href="<?php echo GBH_THEME_URI; ?>/build/singleProduct/singleProduct.css">
    <link rel="stylesheet" href="<?php echo GBH_THEME_URI; ?>/build/singleProduct/singleProduct.css">
    <script type="module" defer fetchpriority="low" src="<?php echo GBH_THEME_URI; ?>/build/singleProduct/singleProduct.bundle.js"></script>
...
<?php while (have_posts()) : the_post(); 
    ...
    if ($main_img) $gallery_images[] = $main_img;
    ...
    if ($acf_gallery && is_array($acf_gallery)) {
        foreach ($acf_gallery as $g_item) {
            ...
            if ($url && !in_array($url, $gallery_images)) $gallery_images[] = $url;
        }
    } else {
        for ($i = 1; $i <= 3; $i++) {
    ...
    <div class="pdp-gallery">
      <div class="pdp-main-img">
        ...
        <?php if ($discount_label): ?>
          <span class="badge-hot" style="position:absolute;top:14px;left:14px;"><?php echo esc_html($discount_label); ?></span>
        <?php else: ?>
...
<section class="care-tabs-section">
  ...
  <div id="tab-growing" class="tab-content active">
    ...
    <?php else: ?>
      <p>Sow seeds directly in pots or grow bags at a depth of 2–2.5 cm. Maintain spacing of 30–45 cm between plants. Keep soil evenly moist during germination. Seeds usually germinate within 7–14 days in favorable weather conditions.</p>
    <?php endif; ?>
  </div>
  ...
  <div id="tab-care" class="tab-content">
    ...
    <?php else: ?>
      ...
      <strong>Soil:</strong> Fertile, well-drained loamy soil with rich organic compost.</p>
    <?php endif; ?>
  </div>
  ...
  <div id="tab-pests" class="tab-content">
    ...
    <?php else: ?>
      <p>Common garden pests include aphids and whiteflies. Spray organic <strong>Neem Oil solution</strong> every 10–15 days as a preventative measure. Ensure proper ventilation around the plant base to avoid fungal diseases.</p>
    <?php endif; ?>
  </div>
  ...
  <div id="tab-harvest" class="tab-content">
    ...
    <?php else: ?>
      <p>Harvest tender fruits/vegetables regularly when they reach optimal size. Continuous harvesting encourages more flowering and fresh yield throughout the growing season.</p>
    <?php endif; ?>
  </div>
</section>
